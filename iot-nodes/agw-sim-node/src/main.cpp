...
void tarea_alertas(void* pv) {
    ...
    for (;;) {
        ...
        bool hay_hdc   = mods.sensor_hdc;
        bool hay_suelo = mods.sensor_suelo;
        bool hay_ph    = mods.sensor_ph;
...
void tarea_sensor_hdc(void* pv) {
    for (;;) {
        ...
        // NODO SIMULADO: temperatura y humedad del aire.
        //
        // Van juntas porque el HDC1080 real entrega las dos en la misma
        // lectura: un nodo que publicara temperatura y dejara la humedad
        // en blanco no se parece a ningun sensor existente.
        //
        // El paseo aleatorio se acota al rango de un cultivo bajo techo.
        // Un valor que oscilara sin limites delataria de inmediato que no
        // sale de ningun sensor.
        if (mods.simulacion && mods.sensor_hdc) {
            if (xSemaphoreTake(xMutexDatos, portMAX_DELAY) == pdTRUE) {
...
void tarea_sensor_suelo(void* pv) {
    ...
    for (;;) {
        ...
        if (mods.simulacion && mods.sensor_suelo) {
            if (xSemaphoreTake(xMutexDatos, portMAX_DELAY) == pdTRUE) {
...
void tarea_sensor_ec(void* pv) {
    ...
    for (;;) {
        ...
        if (mods.simulacion && mods.sensor_ec) {
            if (xSemaphoreTake(xMutexDatos, portMAX_DELAY) == pdTRUE) {
...
void tarea_sensor_ph(void* pv) {
    ...
    for (;;) {
        ...
        if (mods.simulacion && mods.sensor_ph) {
            if (xSemaphoreTake(xMutexDatos, portMAX_DELAY) == pdTRUE) {
